import React from 'react';
import { Dialog } from '@icedesign/base';
import cx from 'classnames';
import { Iconfont } from './iconfont';
import { T } from '../../utils/lang';

export function UiDialog4({ children, header, title, visible, className, onCancel, titleIcon }) {
  return (
    <Dialog className="ui-dialog4" hasMask={false} footer={false} closeable={true} visible={visible}>
      <div className={cx('ui-dialog-content', className)}>
        <div className="ui-dialog-body">
          <div className="ui-dialog-header">
            <Iconfont className="ui-out" onClick={onCancel} icon="out"></Iconfont>
            {titleIcon}
            <div className="ui-dialog-title">{title}</div>
          </div>
          {children}
          <div className="ui-dialog-btns">
            <div className="ui-submit" onClick={onCancel}>
              {T('确 定')}
            </div>
            <div className="ui-cancel" onClick={onCancel}>
              {T('取 消')}
            </div>
          </div>
        </div>
      </div>
    </Dialog>
  );
}
